...
const en = {
  ...
  restartCodex: "Fully quit the Codex desktop host, including its background process, then reopen it. On macOS the host may be named ChatGPT; use ChatGPT → Quit ChatGPT (Cmd-Q). On Windows, exit the desktop app from its tray menu and wait for its Codex helpers to exit. Keep this launcher open, then open the model picker once to verify the catalog.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  restartCodex: "请彻底退出 Codex 桌面宿主（包括后台进程），然后重新打开。macOS 上宿主可能显示为 ChatGPT，请使用“ChatGPT → 退出 ChatGPT”（Cmd-Q）；Windows 上请从托盘菜单退出，并等待 Codex 辅助进程结束。请保持此启动器开启，然后打开一次模型选择器以验证模型目录。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  restartCodex: "Codex デスクトップホストをバックグラウンドプロセスも含めて完全に終了し、もう一度開いてください。macOS ではホスト名が ChatGPT の場合があるため、ChatGPT → Quit ChatGPT（Cmd-Q）を使います。Windows ではトレイメニューから終了し、Codex の補助プロセスが終了するまで待ちます。このランチャーを開いたまま、モデル選択画面を一度開いてカタログを検証してください。",
